...
/*
 TODO: Define a interface for Style.make() from Reason, which
 gets compiled to style({}) in JavaScript.
 ...
 module Style = {
      ...
      external dictToStyle: Js.t(string) => t = "%identity";

      let unsafeAddProp = (style, key, value) => {
        let dict = Js.Unsafe.obj([||]);
        Js.Unsafe.set(dict, key, value);
        combine(style, dictToStyle(dict));
      };
...
/*
 TODO: Bind rules to ReactDOMRe.Style.t
 external unsafeJsonToStyles: Js.json => ReactDOMRe.Style.t = "%identity"; */
